...
$SelectedFormat = isset($_POST["FN"]) ? $_POST["FN"] : $_GET["FN"];
...
#   the base type for the filed.
$TypeMapping = array();
foreach ($Fields as $Field)
{
     $Name = $Server->GetFieldMapping($SelectedFormat, $Field->Name());
     if ($Name !== NULL)
     {
         switch ($Field->Type())
         {
...
foreach ($Fields as $Field)
{
    ...
    if ($Name !== NULL)
    {
        ...
        print("<xs:element name=\"".$Name."\" "
              ...
              .htmlentities($Field->Description())."\n"
